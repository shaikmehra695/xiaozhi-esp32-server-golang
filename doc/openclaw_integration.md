...
3. 在 OpenClaw 控制台角色配置中依次执行以下三条命令：
...
4. 其中 `{url}` 和 `{token}` 使用弹层里复制出的实际值替换。

## 使用方法

1. 在智能体的 `OpenClaw设置` 弹层点击“复制命令”。
2. 在 OpenClaw 控制台角色配置中执行复制出的三条命令，完成 `enabled`、`url`、`token` 配置。
...
- 对话测试超时：检查三条角色配置命令是否执行成功、URL/token 是否正确、OpenClaw 会话是否在线。
